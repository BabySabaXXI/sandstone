...
export function ExaminerSwarm({ essay, question, subject, onComplete, onGenerating }: ExaminerSwarmProps) {
  ...
  // Use refs to track processing state and prevent memory leaks
  ...
  // Memoized feedback generator
  const generateFeedback = useCallback((examinerId: string, score: number, subject: Subject): string => {
  ...
  const getGradeLabel = useCallback((score: number, subject: Subject): string => {
    if (score >= 8) return "A*";
    if (score >= 7) return "A";
    if (score >= 6) return "B";
    if (score >= 5) return "C";
  ...
  const generateSummary = useCallback((score: number, subject: Subject): string => {
  ...
  const generateImprovements = useCallback((score: number, subject: Subject): string[] => {
  ...
  const generateAnnotations = useCallback((essay: string): Annotation[] => {
  ...
  useEffect(() => {
    ...
    const runGrading = async () => {
      ...
      for (let i = 0; i < individualExaminers.length; i++) {
        ...
        currentScores[examiner.id] = clampedScore;
      ...
      const individualScores = Object.entries(currentScores)
      ...
      const result: GradingResult = {
        ...
        examiners: examiners.map((e) => ({
          name: e.name,
          score: currentScores[e.id] || avgScore,
          maxScore: 9,
  ...
  }, [essay, question, subject]); // eslint-disable-line react-hooks/exhaustive-deps
  // Note: onComplete and onGenerating are intentionally omitted to prevent re-triggering
