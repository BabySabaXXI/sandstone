...
const StudyComplete = memo(function StudyComplete({
  ...
  return (
    ...
    >
      ...
      <p className="text-[#5A5A5A] mb-6">You reviewed {total} cards</p>
      ...
      <div className="grid grid-cols-2 gap-4 mb-8">
        ...
        <div className="bg-white rounded-xl border border-[#E5E5E0] p-4">
          <div className="text-3xl font-bold text-[#2D2D2D]">{total}</div>
...
const AllCaughtUp = memo(function AllCaughtUp({ onExit }: AllCaughtUpProps) {
  return (
    <motion.div
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
    ...
    >
      <div className="w-20 h-20 rounded-full bg-[#A8C5A8]/20 flex items-center justify-center mx-auto mb-6">
        <CheckCircle
          className="w-10 h-10 text-[#A8C5A8]"
          aria-hidden="true"
        />
      </div>
...
export const StudyMode = memo(function StudyMode({
  ...
  const { getDeck, getDueCards, reviewCard, setStudyMode } =
    useFlashcardStore();
  ...
  const dueCards = useMemo(
    () => (deck ? getDueCards(deckId) : []),
    [deck, deckId, getDueCards]
  );
  ...
  // Memoized rate handler
  const handleRate = useCallback(
    (quality: number) => {
      if (currentCard) {
        reviewCard(deckId, currentCard.id, quality);
        setSessionStats((prev) => ({
          correct: prev.correct + (quality >= CORRECT_THRESHOLD ? 1 : 0),
          total: prev.total + 1,
        }));
    ...
    [currentCard, currentIndex, deckId, dueCards.length, reviewCard]
  );
    ...
    <div className="max-w-2xl mx-auto">
      ...
      <div className="mt-8 text-center">
        <p className="text-xs text-[#8A8A8A]">
          Press{" "}
          <kbd className="px-1.5 py-0.5 bg-[#F0F0EC] rounded text-[#5A5A5A]">
            Space
          </kbd>{" "}
          to flip
        </p>
