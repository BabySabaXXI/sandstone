/**
 * Grade API Route - Enhanced Version
 ...
 * 
 * Kimi API Key: sk-CkE2F9GDFElGSZSA6NCozb2VKJ6r1qqmR6JoloLOq7ClRe1m
 */
...
import { QuestionType, UnitCode } from "@/lib/examiners/config";
...
async function authenticateRequest(request: NextRequest): Promise<{
  ...
  try {
    ...
    if (error || !user) {
      return { user: null, error: "Unauthorized. Please sign in to use the grading feature.", status: 401 };
    }
...
// ============================================================================
// POST Handler - Main Grading Endpoint
// ============================================================================
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!rateLimit.allowed) {
      ...
      return NextResponse.json(
        { 
          ...
          retryAfter,
          message: `You can make ${RATE_LIMITS.free.maxRequests} grading requests per minute. Please wait ${retryAfter} seconds.`,
        },
        { 
          status: 429,
          headers: {
    ...
    // Perform grading with enhanced service
  ...
  try {
    ...
    if (response.ok) {
      ...
      return NextResponse.json({
        ...
        models: models.data?.map((m: { id: string }) => m.id) || ["kimi-latest"],
      });
    } else {
      return NextResponse.json(
        { 
